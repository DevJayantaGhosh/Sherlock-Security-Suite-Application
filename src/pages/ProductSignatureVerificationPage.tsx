...
export default function SignatureVerificationPage() {
    ...
    const verifySingleRepo = useCallback(async (repoIndex: number) => {
        if (!product || !publicKeyPath || !signaturePath || !window.electronAPI) return false;

        const targetRepo = product.repos[repoIndex];
        const scanId = crypto.randomUUID();
        currentScanId.current = scanId;

        // **KEEP PREVIOUS LOGS** - Just add repo separator + header
        setVerifyLogs(prev => [...prev,
        `\n${"═".repeat(80)}`,
    ...
    }, [product, publicKeyPath, signaturePath]);


    const runSequentialVerification = useCallback(async () => {
        if (!product || !publicKeyPath || !signaturePath || !window.electronAPI) return;
                        ...
                        <motion.div variants={itemVariants}>
                            <Paper sx={{ p: 3, borderLeft: "4px solid #4caf50", borderRadius: 1 }}>
                                ...
                                <Typography variant="body2" color="text.secondary" mb={3}>
                                    Verify cryptographic signatures against repositories using public key (v{product.version})
                                </Typography>
