...
export default function RepoConfigForm({
  ...
  const isFormReady = activeTab === 0 
    ...
    : (isValidLocalPath() && (!hideBranchForLocal ? branch.trim() : true));
  ...
  const FolderPicker = () => (
    <TextField
      fullWidth={!hideBranchForLocal}  // Full width ONLY for quickscan (no branch)
  ...
  return (
    <Paper sx={{ p: 3, borderLeft: `4px solid ${theme.color}`, borderRadius: 1 }}>
      <Stack spacing={2.5}>
        ...
        {/* GITHUB TAB - ALWAYS shows branch */}
        {activeTab === 0 && (
          <Paper sx={{ 
            p: 3, 
        ...
        {activeTab === 1 && (
          <Paper sx={{ 
            ...
            <Box sx={{ 
              display: "grid", 
              gridTemplateColumns: hideBranchForLocal ? "1fr" : "1fr 140px", 
              gap: 2, 
              mb: 2, 
              position: "relative", 
              zIndex: 2 
